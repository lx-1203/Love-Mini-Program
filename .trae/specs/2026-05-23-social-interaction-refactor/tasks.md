...
- [ ] Task 5.1: 全链路端到端验证（Real模式）
  - [ ] SubTask 5.1.1: 微信登录 → 资料编辑 → 资料完善度更新
  - [ ] SubTask 5.1.2: 首页 → 推荐卡片 → 签到 → 每日一问 → 活动推荐 → 村口热门
  - [ ] SubTask 5.1.3: 寻觅 → 喜欢 → 心动信号 → 破冰引导 → 私信
  - [ ] SubTask 5.1.4: 村口 → 发帖 → 评论 → 点赞 → 转发
  - [ ] SubTask 5.1.5: 兴趣圈 → 加入 → 话题 → 回复
  - [ ] SubTask 5.1.6: 签到 → 每日一问 → 回答
  - [ ] SubTask 5.1.7: 活动 → 列表 → 详情 → 报名
  - [ ] SubTask 5.1.8: 临时聊天 → 创建 → 发消息 → 联系方式交换
  - [ ] SubTask 5.1.9: 反馈 → 提交 → 查询历史
  - [ ] SubTask 5.1.10: 推荐偏好设置 → 保存 → 影响推荐结果
  - [ ] SubTask 5.1.11: 在线状态 → 心跳 → 状态标识 → 超时离线
  - [ ] SubTask 5.1.12: 互动提醒 → 事件触发 → 推送通知 → 点击跳转
  - [ ] SubTask 5.1.13: 同校动态 → 帖子/活动/话题聚合 → 浏览
  - **验证**：所有链路在Real模式下正常运行

- [ ] Task 5.2: Mock模式验证
  - [ ] SubTask 5.2.1: 所有Store在Mock模式下正常工作（含新增功能Store）
  - [ ] SubTask 5.2.2: Mock模式下前端可独立运行，无需后端服务
  - **验证**：Mock模式全Store可用

- [ ] Task 5.3: 回归验证
  - [ ] SubTask 5.3.1: 大学生模式（学校认证/同校分类/同校动态）不受影响
  - [ ] SubTask 5.3.2: 资料完善硬门槛正常生效
  - [ ] SubTask 5.3.3: 推荐计划设置完整保留且可影响推荐规则
  - [ ] SubTask 5.3.4: 线下活动功能完整保留
  - [ ] SubTask 5.3.5: 无游戏化元素和购物功能
