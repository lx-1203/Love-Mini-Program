...
async function prepareLogin(mp) {
  try {
...
async function shootPage(mp, p) {
  ...
  if (p.notLogged) {
    ...
    try {
      ...
      await sleep(15000);
    } catch (e) { console.log('  [notlogged] touch err', e.message); }
  } else {
    await prepareLogin(mp);
  }
...
async function main() {
  ...
  const PAGES = [
    ...
    { name: '05-notlogged-waiting', url: '/pages/discover/index', wantTop: true, wantBottom: true, notLogged: true },
    { name: '06-notlogged-profile', url: '/pages/profile/index', wantTop: true, wantBottom: true, notLogged: true },
    { name: '07-messages', url: '/pages/messages/index', wantTop: true, wantBottom: true },
    { name: '08-login', url: '/pages/login/index', wantTop: true, wantBottom: true },
    { name: '09-matching', url: '/pages/discover/matching?preview=1&cardId=1&action=like&userId=2', wantTop: true, wantBottom: true },
    { name: '10-match-success', url: '/pages/discover/match-success?userId=2', wantTop: true, wantBottom: true },
    ...
    { name: '15-profile-other', url: '/pages/profile/other?userId=2', wantTop: true, wantBottom: true },
    { name: '16-topic-detail', url: '/pages/circles/topic-detail?topicId=1', wantTop: true, wantBottom: true },
    { name: '17-campus-hub', url: '/pages/campus/hub', wantTop: true, wantBottom: true },
  ];
