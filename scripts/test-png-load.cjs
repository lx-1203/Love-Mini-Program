...
try {
  const r = await mp.evaluate(() => {
    return new Promise((resolve) => {
      const paths = [
        '/static/assets/images/people/person-07.png',
      ...
      paths.forEach((p, i) => {
        wx.getImageInfo({
          src: p,
          success: (info) => { results[i] = { path: p, ok: true, w: info.width, h: info.height }; done++; if (done===3) resolve(results); },
          fail: (e) => { results[i] = { path: p, ok: false, err: e.errMsg }; done++; if (done===3) resolve(results); }
        });
      });
    });
  });
  console.log('png image info:', JSON.stringify(r));
