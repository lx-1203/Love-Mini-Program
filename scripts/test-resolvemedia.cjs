...
try {
  const r = await mp.evaluate(() => {
    try {
      ...
      const fn = media && (media.resolveMediaUrl || media.default && media.default.resolveMediaUrl);
      if (!fn) return { err: 'no fn', keys: media ? Object.keys(media) : [] };
      return {
        person07: fn('/static/assets/images/people/person-07.png'),
        person02: fn('/static/assets/images/avatars/person-02-avatar.png'),
        avatar1: fn('/static/assets/images/avatars/avatar-1.jpg'),
        empty: fn(''),
        null: fn(null)
      };
    } catch(e) { return { err: e.message }; }
  });
  console.log('resolveMediaUrl test:', JSON.stringify(r));
