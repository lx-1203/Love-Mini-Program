...
await mp.screenshot({path:`${REPO_ROOT}/tmp/profile-v2.png`, fullPage:false});
console.log('profile shot');
// 圈子详情
await mp.reLaunch('/pages/circles/topics?circleId=11');await sleep(16000);
await mp.pageScrollTo(0);await sleep(3000);
await mp.screenshot({path:`${REPO_ROOT}/tmp/circles-topic-v2.png`, fullPage:false});
console.log('circles topic shot');
// 帖子详情
await mp.reLaunch('/pages/village/detail?id=1');await sleep(16000);
await mp.pageScrollTo(0);await sleep(3000);
await mp.screenshot({path:`${REPO_ROOT}/tmp/village-detail-v2.png`, fullPage:false});
