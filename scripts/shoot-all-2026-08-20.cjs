...
/**
 * 全页面截图（2026-08-20，复用 shoot-all-pages-audit.cjs 的稳健逻辑）
 *
 * 对 pages.json 全部路由（主包 + 分包）逐一 reLaunch + 截图 + console 错误采集，
 * 长页追加滚动截图；失败自动重试 1 次，冻结自动恢复。
 *
 * 前置：
 * - dist/build/mp-weixin 已构建（devtools 打开即编译）
 * - 微信开发者工具自动化端口 9420 已开启
 *     cli.bat auto --project <mp-weixin> --auto-port 9420 --trust-project
 *
...
async function main() {
  ...
  console.log(`[${ts()}] 共 ${pages.length} 个路由，启动开发者工具自动化模式 ...`);
  let miniProgram;
  try {
