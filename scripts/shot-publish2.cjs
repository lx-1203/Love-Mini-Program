...
async function main(){
  ...
  try{ await mp.reLaunch('/pages/village/publish?circleId=1'); await new Promise(r=>setTimeout(r,18000)); const p=await mp.currentPage(); console.log('[page]',p&&p.path); await mp.screenshot({path:`${REPO_ROOT}\\tmp\\publish2.png`}); console.log('[OK]'); }catch(e){ console.log('[FAIL]',e.message); }
