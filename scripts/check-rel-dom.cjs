...
// 查询 RelationshipTag 元素
const tags = await page.$$('.relationship-tag');
console.log('relationship-tag elements:', tags.length);
// 查询会话项
const items = await page.$$('.conversation-item');
console.log('conversation items:', items.length);
if (items.length) {
  const first = items[0];
  const text = await first.text();
  console.log('first item text:', JSON.stringify(text).slice(0, 200));
}
