...
async function main(){
  ...
  try{ await mp.reLaunch('/pages/chat-session/index?sessionId=session-1'); await new Promise(r=>setTimeout(r,15000)); await mp.screenshot({path:`${REPO_ROOT}\\tmp\\chat-111-verify.png`}); console.log('[OK] chat'); }catch(e){ console.log('[chat FAIL]',e.message); }
