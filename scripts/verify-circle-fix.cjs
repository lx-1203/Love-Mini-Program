...
await mp.reLaunch('/pages/circles/index');await sleep(18000);
// 验证 image 尺寸
...
try {
  ...
  if (imgs.length > 0) {
    ...
    for (let i=0;i<Math.min(6, imgs.length);i++){
      try { const s = await imgs[i].size(); sizes.push(`img${i}:${JSON.stringify(s)}`); } catch(e){ sizes.push(`img${i}:err`); }
    }
    ...
    console.log('srcs:', srcs.join(' | '));
  }
} catch(e) { console.log('dom err:', e.message); }
await mp.pageScrollTo(0);await sleep(4000);
await mp.screenshot({path:'D:/6/恋爱小程序/tmp/circles-fixed.png', fullPage:false});
