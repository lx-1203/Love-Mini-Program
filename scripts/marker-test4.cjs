...
try { await mp.reLaunch('/pages/circles/index'); } catch(e){ console.log('reLaunch err:', e.message); }
await sleep(20000);
try {
  ...
  console.log('data contains circle-photo at:', i);
  // 找 b 数组
  const m = s.match(/"b":\[(.*?)\]/);
  if (m) {
    const arr = JSON.parse('[' + m[1] + ']');
    const srcs = arr.slice(0,10).map(v=>v && v.b);
    console.log('first 10 srcs:', JSON.stringify(srcs));
  } else {
    console.log('b array not found, head:', s.slice(0, 300));
  }
