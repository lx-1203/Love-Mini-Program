...
try {
  const r = await mp.evaluate(() => {
    try {
      ...
      // 尝试通过页面实例访问 store
      const inst = page && page.$vm;
      if (!inst) return { err: 'no $vm' };
      // 尝试访问注入的 store
      const homeFeed = inst && inst.homeFeed;
      const viewModel = inst && inst.viewModel;
      return {
        hasHomeFeed: !!homeFeed,
        homeFeedKeys: homeFeed ? Object.keys(homeFeed) : [],
        hasToday: !!(homeFeed && homeFeed.todayRecommendation),
        todayName: homeFeed && homeFeed.todayRecommendation ? homeFeed.todayRecommendation.name : null,
        vmKeys: viewModel ? Object.keys(viewModel) : [],
        vmToday: viewModel && viewModel.todayRecommendation ? viewModel.todayRecommendation.name : null
      };
