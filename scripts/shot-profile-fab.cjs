...
async function main(){
  ...
  for (const [name,route] of [['profile','/pages/profile/index']]) {
    try{ await mp.reLaunch(route); await new Promise(r=>setTimeout(r,14000)); await mp.screenshot({path:'D:\\6\\恋爱小程序\\tmp\\profile-fab.png'}); console.log('[OK]',name); }catch(e){ console.log('[FAIL]',name,e.message); }
  }
