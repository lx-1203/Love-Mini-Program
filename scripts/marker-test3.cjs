...
try { const pg = await mp.reLaunch('/pages/circles/index'); console.log('reLaunch page:', pg && pg.path); } catch(e){ console.log('reLaunch err:', e.message); }
await sleep(25000);
try {
  const page = await mp.currentPage();
  console.log('current page:', page && page.path);
