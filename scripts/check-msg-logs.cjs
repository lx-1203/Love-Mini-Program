...
const logs=[];
mp.on('console',(msg)=>{ if(msg&&msg.type==='log') logs.push(String((msg.args||[]).map(a=>typeof a==='string'?a:JSON.stringify(a)).join(' ')).slice(0,200)); });
const d=await post('http://127.0.0.1:8080/api/v1/auth/guest-login','{}');
await mp.callWxMethod('setStorage',{key:'token',data:d.token});
await mp.reLaunch('/pages/messages/index');await sleep(12000);
console.log('logs:', JSON.stringify(logs.filter(l=>l.includes('messages')||l.includes('relationship')||l.includes('dashboard')).slice(0,8),null,2));
