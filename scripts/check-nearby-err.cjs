...
const errs=[];
mp.on('console',(msg)=>{if(msg&&msg.type==='error')errs.push(String((msg.args||[]).map(a=>typeof a==='string'?a:JSON.stringify(a)).join(' ')).slice(0,250))});
const d=await post('http://127.0.0.1:8080/api/v1/auth/guest-login','{}');
await mp.callWxMethod('setStorage',{key:'token',data:d.token});
await mp.reLaunch('/pages/nearby/index');await sleep(12000);
