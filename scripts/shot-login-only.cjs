...
// 只截 08-login
try{
  await mp.reLaunch('/pages/login/index');await sleep(20000);
  await mp.pageScrollTo(0);await sleep(3000);
  const t=`${OUT}/08-login-1-top.png`;
  await mp.screenshot({path:t,fullPage:false});
  console.log('08 top',fs.existsSync(t)?fs.statSync(t).size:0);
}catch(e){console.log('08 ERR',e.message)}
await mp.disconnect();
