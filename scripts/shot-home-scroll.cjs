...
// 滚动到中部（兴趣推荐/附近）
await mp.pageScrollTo(600);await sleep(4000);
await mp.screenshot({path:'D:/6/恋爱小程序/tmp/home-mid.png', fullPage:false});
console.log('mid shot');
// 滚动到底部（社区动态）
await mp.pageScrollTo(99999);await sleep(5000);
await mp.screenshot({path:'D:/6/恋爱小程序/tmp/home-bottom.png', fullPage:false});
console.log('bottom shot');
