...
try {
  ...
  const avatars = await page.$$('.circle-card__friend-avatar');
  console.log('friend avatars:', avatars.length);
  const searchIcons = await page.$$('.circles-nav__search');
  console.log('nav search:', searchIcons.length);
  const tabSearch = await page.$$('.circles-tabs__search');
  console.log('tab search:', tabSearch.length);
} catch(e) { console.log('dom err:', e.message); }
await mp.pageScrollTo(0);await sleep(4000);
await mp.screenshot({path:'D:/6/恋爱小程序/tmp/circles-nav-check.png', fullPage:false});
