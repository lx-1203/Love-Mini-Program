...
--      默认明文密码通过环境变量 ADMIN_PASSWORD 设置（通过环境变量 ADMIN_PASSWORD_HASH 可覆盖哈希值）。
--
-- 注意事项：
--   * BCrypt 哈希格式为 $2a$10$...（cost=10），无法逆推明文。
--   * 默认哈希值由 Flyway placeholder ${admin_password_hash} 提供，
--     可通过环境变量 ADMIN_PASSWORD_HASH 覆盖。
--   * ⚠️ 安全警告：默认密码通过环境变量 ADMIN_PASSWORD 设置，
--     管理员首次登录后必须立即修改密码！
--   * 生成新哈希的方法：
--     调用 com.campuslove.api.config.PasswordEncoderConfig#encodePassword("<你的密码>")
...
--    使用 Flyway placeholder ${admin_password_hash}，默认值见 application-db.yml
