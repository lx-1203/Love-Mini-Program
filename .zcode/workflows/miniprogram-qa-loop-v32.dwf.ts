...
whenToUse: 需要对本微信小程序（apps/client）做全量 QA
  闭环（截图取证→操作验证→六路审查→修复→影响范围回归→独立终验→真实模式前后端联通验收）时运行；改版后回归或发布前验收皆可。
args: {}
*/
// =====================================================================
...
const WORKFLOW = {
  ...
  build: {
    ...
    realScript: "build:mp-weixin:real:dev",   // 本地联动用 real:dev；发布形态才用 build:mp-weixin:real
  ...
  instrument: {
    forbidPlaceholderSelector: true,  // v3.1 的 R2 轮里 288 条 FAILED 有 192 条是 __CAND__ 占位/垃圾标签，属测试台噪声而非产品缺陷
...
const INSTRUMENT_RULES = [
  ...
  `禁止执行器用正则从中文标题里"猜"选择器后回填 ${'__CAND__'} 占位符。实测教训：R2 轮 288 条 FAILED 里 192 条是 ${'__CAND__'}，其中 67 条连 label 都没有、另有 label 被截成"按/校/机/题/赞"等单字垃圾——这些全是测试台缺陷，把它们当产品缺陷会淹没真信号并让通过率彻底失真。`,
  ...
  `【队列对账】启动时打印 SUITES.length 还不够：每完成一个 Suite 追加一行 [queue] done=N running=X pending=[...]（含每套用例数），轮末必须断言 已记录用例数 == 各 Manifest cases[] 总数，不等即为 BLOCKER 并列出缺失套件与缺口条数。实测教训：R2 清单 1107 例、记录 940 例，差 167 例（村口三套未跑），中途两次被误读成"快跑完了/最后一套"。`,
].join("\n");
...
const REAL_E2E_RULES = [
  ...
  `【G8 前后端数据同步五环】按顺序取证，缺一环即 G8 FAIL：(1) 客户端身份：real 模式走"体验账号"按钮 → POST /auth/guest-login 200（wx.login 在本机必 502，见前置条件，记为未取证不记失败）；(2) 写：POST /posts 返回 id；(3) 后台读：GET /admin/forum/village-posts/{id} 命中；(4) 审核驱动可见性：审核前 GET /posts 列表不含该 id（新帖默认 pending，属预期），审核通过后列表 total +1 且该 id 在列；(5) 幂等：同 Idempotency-Key 重放必须 409 且 total 不再变。计数类（评论/赞/藏）要客户端侧与后台侧双向比对一致。`,
  `【G9 real 素材可达性】从 apps/client/src/config/images.ts 解析 IMAGE_PATHS 全量常量，按 real 基址（VITE_API_BASE_URL 去 /api 后缀 + /api/v1/media/app-assets）逐个 GET，断言 0 个 404 且 0 个 0 字节。必须同时报告"抽取到多少 / 探到多少 / 跳过多少"——抽取为 0 是解析失败，不是通过。实测教训：这一项抓到 heart-filled-white.svg 本地存在但 media_asset 0 行，只 real 模式 404，mock 截图永远看不出来。`,
...
if (WORKFLOW.realEnv.requireBackendUp) {
  ...
  if (!health || health.stdout.trim() !== "200") {
    ...
    realGate = await runPnpm(["-C", "apps/client", "run", WORKFLOW.build.realScript], 1800000);
    if (!realGate.ok) blockers.push(`G7 FAIL：${WORKFLOW.build.realScript} 未通过（${realGate.err.slice(-200)}）`);
  }
}
if (realGate.ok) {
  const real = await agent("真实模式验收员", `你负责在本机真实后端上证明"前端写 → 后端落库 → 后台读 → 审核驱动可见性 → 前端读回 → 幂等"这条链路成立，并证明 real 产物自身模式正确、远端素材全部可达。只按契约取证，绝不为了通过而放宽判定；取不到证据就如实 BLOCKED/NOT-EVIDENCED。`).ask<{
  ...
  realGates.push({ gate: "G7 real 构建", result: real.g7.result, evidence: real.g7.evidence });
  realGates.push({ gate: "G8 前后端数据同步", result: real.g8.result, evidence: real.g8.rings.map(r => `${r.name}=${r.ok ? "ok" : "MISS"}`).join(" ") });
  realGates.push({ gate: "G9 real 素材可达性", result: real.g9.result, evidence: `抽取 ${real.g9.extracted} / 探得 ${real.g9.probed} / 跳过 ${real.g9.skipped} / 失败 ${real.g9.failures.length}` });
  for (const g of real.fieldParityGaps) blockers.push(`后台字段对账缺口：${g}`);
  if (real.artifactsLeft.length) log(`真实模式遗留数据 ${real.artifactsLeft.length} 条，待用户决定：${real.artifactsLeft.join(", ")}`);
} else {
...
//   根因：R2 的 288 条 FAILED 里 192 条是执行器用正则从中文标题猜选择器产生的占位/垃圾标签
//         （67 条无 label；label 被截成"按/校/机/题/赞"单字）。按 v3.1 口径这些全算产品失败，
//         通过率失真到无法使用。
...
//   根因：R2 清单 1107 例、实际记录 940 例，村口三套（116 例）从未开跑；启动行只印
...
//
// 未随 v3.2 改动、仍属遗留：产物目录参数化要落在 apps/client/scripts/{prepare-static,
//   prune-unreferenced-static,verify-build-features}.mjs 与 scripts/verify-package-size.mjs
//   （四者硬编码 dist/build/mp-weixin，prune 还会删文件）；这是 G7 能安全原位重建的前置。
// =====================================================================
