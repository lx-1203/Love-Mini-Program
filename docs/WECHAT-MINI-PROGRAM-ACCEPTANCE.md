...
| L10 | SSL 证书申请与部署（主域名+API 子域名） | `docker/client-nginx.conf` 与 `apps/admin/docker/nginx.conf`（R4-02118：仓库无 docker/nginx/conf.d/ 目录，实际 SSL 终止由外层网关/部署机 Nginx 承担，参考上述仓库内配置的 server 块） | 运维-XX | 运维部 | L08、L09（域名需先备案） | 1-2 工作日 | 待线下落实 | verification_logs/2026-07-28-mp-wechat/ssl-certificate.png | 证书颁发机构、到期日期、部署服务器 |
