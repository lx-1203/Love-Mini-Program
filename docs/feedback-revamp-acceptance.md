...
## 第九章：4.1–4.7 验收标准落地（2026-08-05 收尾轮）

### 9.1 寻觅页（4.1）— 全部达标

| 验收项 | 结果 |
|--------|------|
| 默认进入匹配页 | ✅ 寻觅页为首页入口，卡片流默认渲染 |
| 顶部仅标题/剩余次数/搜索/1-2 核心筛选 Chip | ✅ 筛选 Chip 从 4+1 精简为「附近」+「全部筛选」（其余筛选收敛抽屉） |
| 卡片信息完整（头像/昵称/年龄/ID/认证/距离/活跃/资料/描述/标签/MBTI/最新 1-2 条动态） | ✅ 新增「最新动态」预览区（内容 + 点赞/评论计数，最多 2 条） |
| 底部仅 不喜欢/超级喜欢或悄悄话/喜欢 | ✅ 跳过/超级喜欢/喜欢 + 详情页悄悄话 |
| 动态区点赞/评论/私信（私信付费校验） | ✅ 评论真实输入提交（计数 +1）；私信 showModal 付费确认 → 解锁 → 进入会话 |
| 详情页分区：关于我/动态/期待的你/IP 属地 | ✅ 新增 IP 属地分区（字段链：类型→映射→mock→渲染→i18n） |
| 适配 | ✅ H5 大屏限宽 750px 居中（App.vue `#ifdef H5`）；手机端无横向溢出（Chrome 多视口实测） |

### 9.2 首页（4.2）— 全部达标

- ✅ 签到入口保留 + 奖励文案可配置（i18n `checkinReward`，+5 社交币）
- ✅ 功能宫格 5 项（附近的人/兴趣匹配/恋爱咨询/恋爱测试/校园活动）
- ✅ 恋爱咨询二级页（恋爱咨询/恋爱课程/社交咨询/社交课程）
- ✅ 恋爱测试含 MBTI 入口
- ✅ 本周安排默认隐藏（featureFlags）
- ✅ 移除 每日缘分/为你推荐/校园新鲜事/逛逛推荐（区块 + 死代码 + 死样式 + i18n key 清理；banner 缘分文案替换为「发现心动的人」）

### 9.3 消息页（4.3）— 全部达标

- ✅ 官方号 2 个（恋爱助手/活动推送，名称 i18n 可配置）
- ✅ 心动信号 renamed + 倒计时/解锁逻辑正常
- ✅ 喜欢我的/访客入口 + 数量徽标（新增，未解锁也显示数量，点击走解锁提示）
- ✅ 私信列表：搜索 / 置顶（长按 ActionSheet 新增置顶/取消置顶）/ 长按删除

### 9.4 圈子页（4.4）— 全部达标（含双缺陷修复）

| 验收项 | 结果 |
|--------|------|
| 3 Tab（关注/同城/发现） | ✅ |
| 同城 IP 自动城市 + 手动切换 | ✅ 修复：`filteredPosts` getter 委托 `filterAndSortPosts` 统一过滤（原按内容分类精确匹配导致同城/发现恒空）；实测同城 Tab 南京 3 条 |
| 发现二级标签（全部/校友/老乡/搭子圈） | ✅ |
| 帖子卡片（作者/内容/标签/点赞/评论/分享） | ✅ 互动补全（点赞翻转/评论跳详情/分享引导 + aria） |
| 关注流仅匹配喜欢的人 | ✅ isFollowed 语义（mock 空态合理） |
| 论坛机制 | ✅ 发帖/评论/点赞/举报完整（PostReportDialog + detail 页） |
| 帖子加载 | ✅ 修复：isUnlocked 异步变 true 时 watch 补拉（onMounted 错过导致列表恒空） |

### 9.5 我的页（4.5）— 全部达标

- ✅ 无任何 VIP 元素
- ✅ 个人信息区（头像/昵称/认证/学校/签名/编辑资料）
- ✅ 60 秒语音状态（录制/播放/删除）— 语音可用性研究结论：mp-weixin 原生 `uni.getRecorderManager()`（60s duration + 权限 + 事件链完整可用）；H5 为模拟降级
- ✅ 移除个人视频上传与展示（无视频上传 UI；卡片视频角标由 featureFlags 关闭）
- ✅ 菜单：我的动态/任务中心/帮助客服/安全中心/权限/设置
- ✅ 权限开关持久化（localStorage + load() 恢复）并预留后端同步

### 9.6 会员策略（4.6）— 全部达标

- ✅ VIP 购买入口不可见（featureFlags.membershipEnabled=false）
- ✅ 后端会员接口保留，前端不调用
- ✅ 一键重开（featureFlags）

### 9.7 视频/红包下架（4.7 补充）

- ✅ `featureFlags.videoCallEnabled/redPacketEnabled`（默认 false）
- ✅ chat-session 更多菜单入口 + 卡片视频角标全部受开关控制，可一键重开

### 9.8 活动排版自适应

- ✅ 移动端单列流式；H5 宽屏（≥720px）自动双列网格（flex-wrap 增删任意数量对称换行）

### 9.9 开源图库选图研究

- ✅ 盘点：全项目零 CDN 引用，111 个本地 SVG（lucide 风格，stroke currentColor，MIT 可商用）
- ✅ 清理：ICONS_SOCIAL 12 项 + ICONS_PROFILE 4 项 png→svg 切换，删除 11 个冗余 png；实测 profile 页图标全部 SVG 生效

### 9.10 控制后台研究

- ✅ admin（Vue3+TS）11 视图 7 模块；10 个 API 模块 ↔ 后端 15+ Admin*Controller 一一对应
- ✅ 上架流程：管理员登录 → 内容审核（帖子/认证/举报）→ 运营配置（通知/敏感词/匹配）→ 审计留痕；管理员账号初始化 SQL 环境变量可配
- ✅ 可用性：vite dev 启动 + Chrome 实测登录页渲染正常（后端 8080 未启动，登录 E2E 需真实环境）
...
| 单元测试（vitest） | ✅ **86 files / 1166 tests** 全部通过（+4 新增：同城过滤/发现子标签/会话置顶/权限持久化） |
| ESLint | ✅ 0 errors |
| build:mp-weixin | ✅ DONE Build complete |
| 微信开发者工具 automator | ✅ **7/7 页面 PASS，页面异常 0，控制台错误/警告 0**（脚本判定升级：DOM 元素数 + 期望文案，兼容 v2 编译器 data 为空） |
| Chrome MCP（H5） | ✅ 寻觅/首页/消息/圈子/我的 5 页面多视口渲染 + 0 error |
