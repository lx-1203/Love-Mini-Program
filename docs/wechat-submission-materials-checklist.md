...
| 测试账号清单（admin_test/user_test/vip_test） | apps/api/src/main/resources/db/migration V*.sql | - | QA 组 | 待线下落实 | 3 个测试账号，详见本文「测试账号配置说明」小节；对应 L19 |
| 隐私政策文本 | docs/privacy-policy.md | privacy-policy.md | 法务组 | 已就绪 | v1.0.0，2026-07-26 生效；Task 23 补充 8.1 节代码审查记录 |
...
- [x] 隐私政策：docs/privacy-policy.md（v1.0.0，2026-07-26 生效；Task 23 补充 8.1 节代码审查记录）
- [x] 用户协议：docs/user-agreement.md（v1.0.0，2026-07-26 生效）
- [x] 第三方 SDK 列表：docs/third-party-sdks.md（v1.1.0，Task 23 复核：明确 Sentry 仅 H5 启用 + 新增第一方错误上报通道披露）
...
| 账号 | 密码（占位） | 角色 | 功能覆盖范围 | 备注 |
|------|-------------|------|-------------|------|
| `admin_test` | Test@12345 | 管理员 | Admin 后台全功能：用户管理、内容审核、敏感词配置、举报处理、数据统计、通知配置、审计日志 | 用于演示后台运营能力；审核员可登录 Admin 后台验证内容审核流程 |
| `user_test` | Test@12345 | 已认证普通用户 | 微信登录、资料完善、校园认证、推荐匹配、喜欢操作、聊天（文字/语音/图片/视频）、社区话题、活动报名、签到、反馈 | 用于演示核心用户旅程；已通过校园认证（学生证/邮箱），可体验匹配与聊天全流程 |
| `vip_test` | Test@12345 | VIP 用户 | 普通用户全部功能 + VIP 权益：高级筛选、无限喜欢、访客记录、专属徽章、红包功能、视频通话、推广码 | 用于演示 VIP 商业化能力；已购买月度 VIP（18 元/月），可体验付费功能 |

> **测试数据要求**：
> - `user_test` 与 `vip_test` 需有已发布的动态/话题，用于演示社区互动
> - `user_test` 与 `vip_test` 之间需有匹配关系，用于演示聊天功能
> - `vip_test` 需有 VIP 账单记录，用于演示 VIP 权益
...
4. **逐个添加测试账号**（共 3 个）：
   - 账号 1：填入 `admin_test` / 密码 `Test@12345` / 备注「管理员账号，可登录 Admin 后台」
   - 账号 2：填入 `user_test` / 密码 `Test@12345` / 备注「已认证普通用户，可体验匹配与聊天」
   - 账号 3：填入 `vip_test` / 密码 `Test@12345` / 备注「VIP 用户，可体验付费功能」
5. **填写测试说明**：在「测试说明」文本框中填写：
   ...
   3. vip_test（VIP 用户）：已购买 VIP，可体验高级筛选、无限喜欢、红包等付费功能
   
   密码均为 Test@12345，登录后请勿修改密码或删除测试数据。
   ```
...
| 账号 | 需准备的测试数据 | 数据来源 |
|------|-----------------|----------|
| `admin_test` | 已处理的举报记录 5 条、已审核的内容 10 条、敏感词配置完整 | 通过 Admin 后台手动创建或 Flyway 迁移脚本注入 |
| `user_test` | 已完善的资料（头像/昵称/性别/学校/兴趣标签）、已发布的动态 3 条、已参与的话题 2 个 | 通过小程序端注册并操作，或 Flyway 迁移脚本注入 |
| `vip_test` | user_test 全部数据 + VIP 账单 1 条（18 元/月）、推广码 1 个、红包记录 1 条 | 通过小程序端购买 VIP 或 Flyway 迁移脚本注入 |

> **测试数据注入方式**：通过 Flyway 迁移脚本 `apps/api/src/main/resources/db/migration/V*.sql` 在测试环境初始化测试账号与测试数据。生产环境不注入测试数据，测试账号仅在预发布环境可用。
...
```
【小程序名称】校园恋爱
【小程序简介】面向高校在校学生的校园社交小程序，提供基于校园认证的异性匹配、即时聊天、社区话题、活动报名、VIP 会员等服务。
【主体类型】企业主体（已认证）
【服务类目】社交 > 社交资讯
【版本号】1.0.0（商业化首版）
【核心功能】
1. 微信登录与校园认证（学生证/教育邮箱双重验证，仅限在校大学生）
2. 资料完善与异性推荐匹配（基于学校、年龄、兴趣标签）
3. 即时聊天（文字/语音/图片/视频，支持消息举报与撤回）
4. 社区话题与动态发布（含敏感词过滤 + 机审 + 人审）
5. 活动报名与签到（线下校园活动）
6. VIP 会员（微信支付，月卡 18 元 / 季卡 48 元 / 年卡 168 元）
7. 举报与申诉（用户/内容/消息三类举报，7 天申诉期）
8. 个人中心（资料管理、订单、隐私设置、账号注销）
【未成年人保护】注册时通过校园认证校验在校大学生身份，未成年人无法通过认证；隐私政策第 8 条明确未成年人保护条款。
【内容审核机制】敏感词过滤（6 大类）+ AI 图片视频识别 + 人工复核 + 用户举报 + 违规处罚（警告/7天/30天/永久）。
```
...
3. vip_test（VIP 用户）
   密码：Test@12345
   权限：user_test 全部功能 + VIP 权益（高级筛选、无限喜欢、访客记录、专属徽章、红包功能、视频通话、推广码）
   说明：已购买月度 VIP（18 元/月）
...
【未成年人保护】
- 注册时通过校园认证校验在校大学生身份，未成年人无法通过认证
- 隐私政策第 8 条明确未成年人保护条款
- 隐私政策 8.1 节披露 RealAuthService.loginWithWechat 与 RealCampusCertificationService.submitCertification 的代码审查记录
...
3. **AppID 不一致问题**：当前项目中存在两个不同的 appid，需在 T-10 前由运营/产品确认正式 appid 并统一替换，否则微信开发者工具将报"appid 不匹配"警告。
