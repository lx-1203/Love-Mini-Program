...
public class AppleIdentityTokenVerifier {
    ...
    public String verifyAndGetSubject(String identityToken) {
        if (identityToken == null || identityToken.isBlank()) {
            throw new IllegalArgumentException("identityToken 不能为空");
        }
        ...
        if (publicKey == null) {
            throw new IllegalArgumentException("无法获取 Apple 公钥（kid=" + kid + "）");
        }
        ...
        try {
            ...
            throw new IllegalArgumentException("Apple identityToken 签名无效");
        } catch (io.jsonwebtoken.JwtException | IllegalArgumentException e) {
            log.warn("Apple identityToken 声明校验失败: {}", e.getMessage());
            throw new IllegalArgumentException("Apple identityToken 无效: " + e.getMessage());
        }
        ...
        if (claims.getExpiration() != null && claims.getExpiration().toInstant().isBefore(now)) {
            throw new IllegalArgumentException("Apple identityToken 已过期");
        }
        ...
        if (expectedNonce != null && !expectedNonce.isBlank()) {
            ...
            if (nonce == null || !expectedNonce.equals(nonce.toString())) {
                throw new IllegalArgumentException("Apple identityToken nonce 校验失败");
            }
        }

        String sub = claims.getSubject();
        if (sub == null || sub.isBlank()) {
            throw new IllegalArgumentException("Apple identityToken 缺少 sub");
        }
        return sub;
    }
    ...
    private String parseKid(String token) {
        try {
            ...
            if (parts.length != 3) {
                throw new IllegalArgumentException("identityToken 不是合法 JWT");
            }
            String headerJson = new String(Base64.getUrlDecoder().decode(parts[0]));
            // 轻量解析 kid（避免引入额外 JSON 库依赖）
            int kidIdx = headerJson.indexOf("\"kid\"");
            if (kidIdx < 0) {
                throw new IllegalArgumentException("identityToken header 缺少 kid");
            }
            ...
            throw new IllegalArgumentException("identityToken 格式非法", e);
        }
    }
