...
/** 第三方账号绑定状态（从会话登录方式推导，保证与登录页顶部登录方式一致） */
const wechatBound = computed(() => sessionStore.userSession?.loginMethod === "wechat");

/**
 * 第三方账号绑定操作（演示环境）。
 * - 微信已绑定（当前登录方式）时点击 → 二次确认，演示环境不支持解绑；
 * - 未绑定 → 提示通过快捷登录方式切换账号（登录后自动绑定）。
 */
function onThirdPartyBindTap(): void {
  lightHaptic();
  if (wechatBound.value) {
    uni.showModal({
      title: t("thirdPartyLogin.unbindWechat"),
      content: t("thirdPartyLogin.unbindConfirm"),
      confirmText: t("thirdPartyLogin.unbindWechat"),
      cancelText: t("common.cancel"),
      confirmColor: designTokens.color.error,
      success: (res) => {
        if (res.confirm) {
          uni.showToast({ title: t("security.thirdPartyDemoHint"), icon: "none" });
        }
      },
    });
    return;
...
<template>
  <AppShell :title="t('security.navTitle')" :subtitle="t('security.subtitle')" show-back>
    ...
    <!-- 第三方账号绑定（绑定状态与会话登录方式一致） -->
    <SectionCard :title="t('security.thirdPartyTitle')" compact>
      <view class="sec-list">
        <view
          class="sec-item press-feedback"
          hover-class="press-feedback--active"
          hover-stay-time="40"
          role="button"
          :aria-label="t('thirdPartyLogin.bindWechat')"
          @tap="onThirdPartyBindTap"
        >
          <view class="sec-item__icon-wrap" :style="{ background: 'var(--c-tint-green-soft, #E8FAF3)' }">
            <image class="sec-item__icon" :src="IMAGE_PATHS.ICONS_V2.WECHAT_GREEN_SVG" mode="aspectFit" alt="" />
          </view>
          <view class="sec-item__info">
            <text class="sec-item__label">{{ t('thirdPartyLogin.bindWechat') }}</text>
            <text class="sec-item__desc">{{ wechatBound ? t('thirdPartyLogin.alreadyBound') : t('thirdPartyLogin.notBound') }}</text>
          </view>
          <text class="sec-item__arrow">›</text>
        </view>

        <view class="sec-item">
          <view class="sec-item__icon-wrap sec-item__apple-icon-wrap" aria-hidden="true">
            <view class="sec-item__apple-icon" />
          </view>
          <view class="sec-item__info">
            <text class="sec-item__label">{{ t('thirdPartyLogin.bindApple') }}</text>
            <text class="sec-item__desc">{{ t('thirdPartyLogin.appleNotSupported') }}</text>
          </view>
          <text class="sec-item__arrow">›</text>
        </view>
      </view>
...
/* Apple 图标（内联 SVG 背景，避免额外图片资源；圆形深色底 + 白色 logo） */
.sec-item__apple-icon-wrap {
  background: var(--c-neutral-900);
}

.sec-item__apple-icon {
  width: 40rpx;
  height: 40rpx;
  background-image: url("data:image/svg+xml;utf8,%3Csvg%20xmlns%3D%22http%3A//www.w3.org/2000/svg%22%20viewBox%3D%220%200%2024%2024%22%20fill%3D%22white%22%3E%3Cpath%20d%3D%22M16.365%201.43c0%201.14-.493%202.27-1.177%203.08-.744.9-1.99%201.57-2.987%201.57-.12%200-.23-.02-.3-.03-.01-.06-.04-.22-.04-.39%200-1.15.572-2.27%201.206-2.98.804-.94%202.142-1.64%203.248-1.68.03.13.05.28.05.43zm4.565%2015.71c-.03.07-.463%201.58-1.518%203.12-.945%201.34-1.94%202.71-3.43%202.71-1.517%200-1.9-.88-3.63-.88-1.698%200-2.302.91-3.67.91-1.377%200-2.332-1.26-3.428-2.8-1.287-1.82-2.323-4.63-2.323-7.28%200-4.28%202.797-6.55%205.552-6.55%201.448%200%202.675.95%203.6.95.865%200%202.222-1.01%203.902-1.01.632%200%202.93.06%204.43%202.19-.114.07-2.402%201.37-2.402%204.13%200%203.27%202.866%204.42%202.967%204.45z%22/%3E%3C/svg%3E");
