...
/** 状态栏高度（px）：根节点注入 --statusbar 供自定义导航避让状态栏 */
...
/** 2026-08-26 R4：详情接口加载中（无列表缓存时的首帧骨架屏） */
const detailLoading = ref(false);

/**
 * 2026-08-27 理想图：发布者分类标签（如「摄影圈」）。
 * 优先取 store 已加载的分类名（villageStore.categories，mock/real 均可），
 * 未加载时回退到本地分类映射，避免直入详情（如首页社区动态）时标签为空。
 */
const POST_CATEGORY_FALLBACK_LABELS: Record<string, string> = {
  "cat-sincere": "诚意帖",
  "cat-interest": "兴趣圈",
  "cat-campus": "校园圈",
  "cat-hometown": "同乡圈",
...
const categoryLabel = computed<string>(() => {
  ...
  const found = villageStore.categories.find((c) => c.id === categoryId);
  if (found?.name) return found.name;
  return POST_CATEGORY_FALLBACK_LABELS[categoryId] ?? "";
});
...
onLoad((query) => {
  ...
  if (postId && typeof postId === "string" && postId.trim().length > 0) {
    ...
    if (!currentPost.value || currentPost.value.id !== postId) {
      // 2026-08-26 R4：首帧先用列表缓存（villageStore.posts 同 id 帖子）立即渲染
      // 标题/作者/正文，详情接口返回后替换——消除白屏等待；无缓存时显示骨架屏
      const cached = villageStore.posts.find((p) => p.id === postId);
      if (cached) {
        villageStore.currentPost = cached;
        villageStore.comments = [];
      } else {
...
<template>
  ...
  <view class="detail-page">
    ...
    <scroll-view v-if="currentPost" class="detail-body" scroll-y>
      ...
      <view class="author-inline">
        ...
        <view class="author-inline__info">
          ...
          <!-- 频道 · 时间（2026-08-27 理想图：分类标签如「摄影圈」· 相对时间） -->
          <text class="author-inline__meta">
            <text v-if="categoryLabel" class="author-inline__category">{{ categoryLabel }}</text>
            <text v-if="categoryLabel" class="author-inline__meta-sep">·</text>
    ...
    <!-- 2026-08-26 R4：详情加载骨架屏（无列表缓存时的首帧占位，避免白屏/闪错误态） -->
    <view v-else-if="detailLoading" class="detail-skeleton" aria-label="loading">
      <view class="detail-skeleton__header">
        <view class="detail-skeleton__avatar" />
        <view class="detail-skeleton__author">
          <view class="detail-skeleton__line detail-skeleton__line--name" />
          <view class="detail-skeleton__line detail-skeleton__line--meta" />
        </view>
      </view>
      <view class="detail-skeleton__body">
      ...
      <view class="detail-skeleton__block" />
    </view>
...
.detail-page {
  ...
  --c-brand-200: #A3EBCF;
  --c-romance-50: #FFECEF;
  --c-romance-100: #FFD9DF;
  --c-romance-200: #FFB3C0;
}
...
.detail-skeleton__line--name {
  width: 40%;
}

.detail-skeleton__line--meta {
  width: 60%;
}
...
/* 2026-08-27 理想图：作者行分类标签（如「摄影圈」绿色胶囊） */
.author-inline__category {
  display: inline-flex;
  align-items: center;
  padding: 2rpx 14rpx;
  margin-right: 8rpx;
...
.author-inline__follow {
  padding: 10rpx 28rpx;
  border-radius: var(--r-full, 9999rpx);
  /* 2026-08-27 理想图：关注按钮统一绿描边 + 白色底（与帖子卡片 follow-chip 一致） */
