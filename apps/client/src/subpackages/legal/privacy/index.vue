...
async function loadPrivacyPolicy(): Promise<void> {
  ...
  errorMessage.value = "";
  try {
    const result = await getLegalText(LegalTextType.PRIVACY_POLICY);
...
const displayTitle = computed<string>(() => {
  return legalText.value?.title || t("legal.privacyPolicy.title");
});
      ...
      <view class="legal-header">
        ...
        <view v-if="showMeta" class="legal-header__meta">
          ...
          <text v-if="displayUpdatedAt" class="legal-header__updated">
            {{ t("legal.privacyPolicy.lastUpdated") }}: {{ displayUpdatedAt }}
          </text>
        </view>
      </view>
