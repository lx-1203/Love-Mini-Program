 ...
 * {"title":"活动标题","desc":"描述","tag":"标签","targetUrl":"/subpackages/tools/activities/detail?id=xxx"}
 *
...
onShow(() => {
  ...
  addBreadcrumb("navigation", "page_enter", {
    url: "/subpackages/chat/chat-session/index",
    sessionId: sessionId.value,
  });
...
const peerAvatarPool = [
  // 2026-08-26 P1-2：首项由 AVATAR_1 改为 AVATAR_4——
  // AVATAR_1(person-01) 与 self 真实头像可能同图，避免 hash 落 0 时自他头像同名错位
...
const peerAvatarSrc = computed(() => {
  ...
  // 2026-08-26 P1-2：默认回退 AVATAR_3（与 self 真实头像 person-01/AVATAR_1 区分，避免自他同名错位）
  return IMAGE_PATHS.AVATARS.AVATAR_3;
});
