...
function goUserProfile(user: UserSearchView) {
  openAppPath(`/pages/profile/other?userId=${encodeURIComponent(String(user.userId))}`);
}
        ...
        <view v-if="campusExpanded" class="campus-preview">
          ...
          <view v-if="campusPeople.length > 0" class="campus-preview__users">
            <view
              ...
              @tap="openAppPath(`/pages/profile/other?userId=${encodeURIComponent(person.userId)}`)"
            >
