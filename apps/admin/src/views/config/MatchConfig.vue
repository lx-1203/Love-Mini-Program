...
/**
 * Admin 匹配与推荐参数配置视图（复制自旧后台 apps/admin，适配 admin-v2 目录结构）。
 *
...
/** 是否全局超级管理员（写操作仅 SUPER_ADMIN 可用，普通管理员保存会被后端 403） */
 ...
 * 与 error 分离：ErrorState 的"重试"绑定 fetchAll 会重拉数据并覆盖用户全部未保存编辑。
 */
...
<template>
  <view class="match-page">
    ...
    <!-- 校区管理员只读提示：写操作仅超级管理员可用 -->
    <view v-if="!isSuperAdmin" class="readonly-tip" role="status">
      <text>{{ t("matchConfig.readonlyTip") }}</text>
    </view>
